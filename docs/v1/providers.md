...
Provider IDs are logical identifiers. They MUST remain stable across compatible releases and MUST NOT derive from an install path. The final lexical/registry policy remains open. Breaking payload interpretation requires a new payload version or provider identity.
