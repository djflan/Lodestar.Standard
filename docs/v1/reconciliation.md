...
| Sequences, tempo maps, transport, arrangements, automation lanes, and MIDI mappings remain host/editor state | Complete | [events](events.md) explicitly excludes them from Performance and `.star` serialization. |
| Native Event → Part → Instrument → Layers → Sources → Providers | Complete | [events](events.md), `ILodestarEvent.PartId`, event diagram. |
| MIDI is a host adapter, not a native model or legacy-profile compatibility promise | Complete | [events](events.md), [host boundaries](host-boundaries.md). |
...
| `.star` ZIP; Standard version separate from `packageVersion` | Complete | [packages](packages.md), manifest schemas/contracts. |
...
| Fixed prepared latency; host-owned compensation and latency-preserving bypass | Complete | [realtime](realtime.md); no normative hard latency limit. |
...
| `object-model.md` | Complete | Ownership, cardinality, runtime split, canonical output, and host-state boundary. |
...
| `events.md` | Complete | Part targeting, runtime control addresses, and host sequencing/automation boundary. |
...
| `decisions-and-open-questions.md` | Complete | Settled v1 boundaries and genuine Draft choices. |
...
| `performance.schema.json` | Complete | Parts, Mixer, Master, and canonical main stereo output. |
...
|---|---|---|
| `Assets/AssetContracts.cs` | Complete | Hierarchy, distinct channel/mixer roles, and canonical output. |
...
| `Packages/PackageContracts.cs` / `StarPackage.cs` | Complete | Manifest/assets/resources/dependencies/requirements/`packageVersion`/extension. |
...
- Removed portable sequencing, DAW automation, MIDI mappings, and auxiliary outputs from v1 content; hosts may retain such state independently.
